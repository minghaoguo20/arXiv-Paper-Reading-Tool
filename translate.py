...
def fix_package_conflicts(output_dir: Path) -> None:
    ...
    # First check if any file uses CJK package
    has_cjk = r'\usepackage{CJK}' in all_content
    if has_cjk:
        for tex_file in all_tex_files:
            try:
                content = tex_file.read_text(encoding='utf-8', errors='replace')
                original = content
                # Comment out old CJK package
                if r'\usepackage{CJK}' in content:
                    content = content.replace(
                        r'\usepackage{CJK}',
                        r'%\usepackage{CJK} % replaced by xeCJK'
                    )
                # Replace \begin{CJK}{UTF8}{gbsn}...\end{CJK} with just the content (in all files)
...
def process_paper(paper_dir: Path) -> None:
    ...
    # Process section files or main file
    sections_dir = output_dir / "sections"
    if sections_dir.exists():
        section_files = sorted(sections_dir.glob("*.tex"))
        for section_file in tqdm(section_files, desc="Translating sections"):
            print(f"  {section_file.name}")
            content = section_file.read_text(encoding='utf-8')
            translated_content = translate_section_file(content)
            section_file.write_text(translated_content, encoding='utf-8')
    else:
        # No sections directory - translate main file directly
